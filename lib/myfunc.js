...
export const smsg = (SubhanDev, m, store) => {
    if (!m)
        return m;
    ...
    if (m.key) {
        ...
        m.sender = SubhanDev.decodeJid(m.fromMe && SubhanDev.user.id || m.participant || m.key.participant || m.chat || '');
        if (m.isGroup)
            m.participant = SubhanDev.decodeJid(m.key.participant) || '';
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.sender = SubhanDev.decodeJid(m.msg.contextInfo.participant);
            m.quoted.fromMe = m.quoted.sender === (SubhanDev.user && SubhanDev.user.id);
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || '';
            m.quoted.mentionedJid = m.msg.contextInfo ? m.msg.contextInfo.mentionedJid : [];
            m.getQuotedObj = m.getQuotedMessage = async () => {
                if (!m.quoted.id)
                    return false;
                const q = await store.loadMessage(m.chat, m.quoted.id, SubhanDev);
                return smsg(SubhanDev, q, store);
            };
            ...
            m.quoted.delete = () => SubhanDev.sendMessage(m.quoted.chat, { delete: vM.key });
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => SubhanDev.copyNForward(jid, vM, forceForward, options);
            m.quoted.download = () => SubhanDev.downloadMediaMessage(m.quoted);
        }
    }
    if (m.msg?.url)
        m.download = () => SubhanDev.downloadMediaMessage(m.msg);
    m.text = m.msg?.text || m.msg?.caption || m.message?.conversation || m.msg?.contentText || m.msg?.selectedDisplayText || m.msg?.title || '';
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text)
        ? SubhanDev.sendMedia(chatId, text, 'file', '', m, { ...options })
        : SubhanDev.sendText(chatId, text, m, { ...options });
    m.copy = () => smsg(SubhanDev, M.fromObject(M.toObject(m)), store);
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => SubhanDev.copyNForward(jid, m, forceForward, options);
    return m;
};
