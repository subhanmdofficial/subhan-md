...
global.botname = config.botName || "𝐒ᴜʙʜᴀɴ-𝐌ᴅ";
global.themeemoji = "•";
...
async function startSubhanDev() {
    try {
        ...
        SubhanDev.store = store;
        const originalSendPresenceUpdate = SubhanDev.sendPresenceUpdate;
        const originalReadMessages = SubhanDev.readMessages;
        const originalSendReceipt = SubhanDev.sendReceipt;
        SubhanDev.sendPresenceUpdate = async function (...args) {
            const ghostMode = await store.getSetting('global', 'stealthMode');
        ...
        SubhanDev.readMessages = async function (...args) {
            const ghostMode = await store.getSetting('global', 'stealthMode');
            if (ghostMode && ghostMode.enabled)
                return;
        ...
        if (originalSendReceipt) {
            SubhanDev.sendReceipt = async function (...args) {
                const ghostMode = await store.getSetting('global', 'stealthMode');
                if (ghostMode && ghostMode.enabled)
                    return;
        ...
        const originalQuery = SubhanDev.query;
        SubhanDev.query = async function (node, ...args) {
            const ghostMode = await store.getSetting('global', 'stealthMode');
            if (ghostMode && ghostMode.enabled) {
        ...
        SubhanDev.isGhostMode = async () => {
            const ghostMode = await store.getSetting('global', 'stealthMode');
            return ghostMode && ghostMode.enabled;
        };
        SubhanDev.ev.on('creds.update', _saveCreds);
        store.bind(SubhanDev.ev);
        SubhanDev.ev.on('messages.upsert', async (chatUpdate) => {
            try {
                ...
                if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                    await handleStatus(SubhanDev, chatUpdate);
                    return;
                }
                if (!SubhanDev.public && !mek.key.fromMe && chatUpdate.type === 'notify') {
                ...
                if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16)
                    return;
                if (SubhanDev?.msgRetryCounterCache) {
                    SubhanDev.msgRetryCounterCache.clear();
                }
                try {
                    await handleMessages(SubhanDev, chatUpdate);
                }
                catch (err) {
                    printLog('error', `Error in handleMessages: ${err.message}`);
                    if (mek.key && mek.key.remoteJid) {
                        await SubhanDev.sendMessage(mek.key.remoteJid, {
        ...
        SubhanDev.ev.on('contacts.update', (update) => {
            for (const contact of update) {
                const id = SubhanDev.decodeJid(contact.id);
                if (store && store.contacts)
                    store.contacts[id] = { id, name: contact.notify };
            }
        });
        SubhanDev.getName = (jid, withoutContact = false) => {
            const id = SubhanDev.decodeJid(jid);
            withoutContact = SubhanDev.withoutContact || withoutContact;
            let v;
            if (id.endsWith("@g.us"))
                return new Promise(async (resolve) => {
                    v = store.contacts[id] || {};
                    if (!(v.name || v.subject))
                        v = SubhanDev.groupMetadata(id) || {};
        ...
        if (pairingCode && !isRegistered) {
            ...
            const doPairing = async (num, attempt = 1) => {
                try {
                    let code = await SubhanDev.requestPairingCode(num);
        ...
        SubhanDev.ev.on('connection.update', async (s) => {
            ...
            if (connection === "open") {
                ...
                printLog('success', `Connected to => ${ JSON.stringify(SubhanDev.user, null, 2)}`);
                try {
                    const botNumber = `${SubhanDev.user.id.split(':')[0] }@s.whatsapp.net`;
                    const ghostStatus = (ghostMode && ghostMode.enabled) ? '\n👻 Stealth Mode: ACTIVE' : '';
                    await SubhanDev.sendMessage(botNumber, {
                ...
                printLog('info', `[ ${config.botName || '𝐒ᴜʙʜᴀɴ-𝐌ᴅ'} ]`);
        ...
        SubhanDev.ev.on('call', async (calls) => {
            await handleCall(SubhanDev, calls);
        });
        SubhanDev.ev.on('group-participants.update', async (update) => {
            await handleGroupParticipantUpdate(SubhanDev, update);
        });
        SubhanDev.ev.on('status.update', async (status) => {
            await handleStatus(SubhanDev, status);
        });
        SubhanDev.ev.on('messages.reaction', async (reaction) => {
            await handleStatus(SubhanDev, reaction);
        });
        return SubhanDev;
    }
    catch (error) {
        printLog('error', `Error in startSubhanDev: ${error.message}`);
        if (rl && !rlClosed) {
            rl.close();
            rl = null;
        }
...
async function main() {
    ...
    printLog('info', 'Starting 𝐒ᴜʙʜᴀɴ-𝐌ᴅ BOT...');
    await initializeSession();
    await delay(3000);
    startSubhanDev().catch((error) => {
